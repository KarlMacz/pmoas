...
@section('content')
    ...
    <div class="container-spacious">
        ...
        <form action="{{ route('employees.post.employees_register') }}" method="POST">
            {{ csrf_field() }}
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                        <label for="email-field">E-mail Address:</label>
                        <input type="email" name="email" id="email-field" class="form-control" value="{{ old('email') }}" placeholder="E-mail Address" required autofocus>
                        @if ($errors->has('email'))
                    ...
                    <div class="form-group{{ $errors->has('position') ? ' has-error' : '' }}">
                        ...
                        <select name="position" class="form-control" id="position-field" required>
                            ...
                            <optgroup label="Employee">
                                <option value="Employee - Logistics Department"{{ (old('position') === 'Employee - Logistics Department' ? ' selected' : '') }}>Logistics Department</option>
                                <option value="Employee - Sales Department"{{ (old('position') === 'Employee - Sales Department' ? ' selected' : '') }}>Sales Department</option>
                            </optgroup>
